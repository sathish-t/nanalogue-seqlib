...
/* Invoke legacy mutable callbacks through their actual function type. */
static int enc_aux_cb(int op, const ASN1_VALUE **pval, const ASN1_ITEM *it)
{
    const ASN1_AUX *aux = it->funcs;
    if (aux == NULL)
        return 1;
    if (aux->flags & ASN1_AFLG_CONST_CB)
        return aux->asn1_const_cb == NULL || aux->asn1_const_cb(op, pval, it, NULL);
    return aux->asn1_cb == NULL || aux->asn1_cb(op, (ASN1_VALUE **)pval, it, NULL);
}
    ...
    const ASN1_ITEM *it, int tag, int aclass)
{
    ...
    case ASN1_ITYPE_CHOICE:
        ...
        if (!enc_aux_cb(ASN1_OP_I2D_PRE, pval, it))
            return 0;
        ...
        if (!enc_aux_cb(ASN1_OP_I2D_POST, pval, it))
            return 0;
        break;
